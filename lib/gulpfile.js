...
function compile(modules) {
  ...
  const sass = gulp
    ...
    .pipe(
      through2.obj(function (file, encoding, next) {
        ...
        transformSass(cloneCssFile.path)
          .then(css => {
            cloneCssFile.contents = Buffer.from(css);
            cloneCssFile.path = cloneCssFile.path.replace(/\.s(a|c)ss$/, '.css');
  ...
  // ================================ TS ================================
  const source = ['src/**/*.tsx', 'src/**/*.ts', 'typings/**/*.d.ts', '!src/**/__tests__/**'];
  ...
  return merge2([sass, less, tsFilesStream, tsd, assets, transformFileStream].filter(s => s));
}

function publish(tagString, done) {
  let args = ['publish', '--with-taror-tools', '--access=public'];
  if (tagString) {
    ...
    if (npmArgs) {
      for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
        if (
          ...
          npmArgs.indexOf('--with-taror-tools') < 0 &&
